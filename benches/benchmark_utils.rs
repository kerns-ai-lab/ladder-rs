...
impl TestDataGenerator {
    ...
    pub fn generate_elo_ratings(&mut self, count: usize, distribution: SkillDistribution) -> Vec<EloRating> {
        match distribution {
            SkillDistribution::Uniform => {
                (0..count).map(|_| EloRating::new(1500.0)).collect()
            }
    ...
    pub fn generate_trueskill_ratings(&mut self, count: usize, distribution: SkillDistribution) -> Vec<TrueSkillRating> {
        match distribution {
            SkillDistribution::Uniform => {
                (0..count).map(|_| TrueSkillRating::new(25.0, (25.0_f64/3.0).powi(2)).unwrap()).collect()
            }
            SkillDistribution::Normal => {
                (0..count).map(|_| {
            ...
            SkillDistribution::Bimodal => {
                (0..count).map(|_| {
                    let (mean, variance) = if self.rng.gen_bool(0.3) {
                        (self.rng.gen_range(35.0..45.0), self.rng.gen_range(25.0..64.0)) // Experts
                    } else {
                        (self.rng.gen_range(5.0..15.0), self.rng.gen_range(64.0..144.0))  // Beginners
                    };
                    TrueSkillRating::new(mean, variance).unwrap()
                }).collect()
            }
            ...
            SkillDistribution::Extreme => {
                (0..count).map(|_| {
                    let (mean, variance) = if self.rng.gen_bool(0.1) {
                        (self.rng.gen_range(45.0..60.0), self.rng.gen_range(16.0..36.0)) // Super experts
                    } else {
                        (self.rng.gen_range(1.0..5.0), self.rng.gen_range(100.0..400.0)) // Very weak
                    };
                    TrueSkillRating::new(mean, variance).unwrap()
                }).collect()
            }
    ...
    pub fn generate_outcomes(&mut self, games: usize, players_per_game: usize, pattern: OutcomePattern) -> Vec<GameOutcome> {
        (0..games).map(|_| {
            match pattern {
