...
pub fn bench_rating_creation(c: &mut Criterion) {
    ...
    group.bench_function("elo_system", |b| b.iter(|| black_box(EloSystem::new())));

    group.bench_function("elo_rating", |b| {
        b.iter(|| black_box(EloRating::new(1500.0)))
    });

    group.bench_function("glicko_system", |b| b.iter(|| black_box(Glicko::new())));

    group.bench_function("glicko2_system", |b| b.iter(|| black_box(Glicko2::new())));
...
pub fn bench_single_game_update(c: &mut Criterion) {
    ...
    group.bench_function("elo", |b| {
        b.iter(|| {
            black_box(
                elo_system
                    .rate(&[elo_team1.clone(), elo_team2.clone()], &outcome)
                    .unwrap(),
            )
        })
    });
    ...
    group.bench_function("glicko", |b| {
        b.iter(|| {
            black_box(
                glicko_system
                    .rate(&[glicko_team1.clone(), glicko_team2.clone()], &outcome)
                    .unwrap(),
            )
        })
    });
    ...
    group.bench_function("glicko2", |b| {
        b.iter(|| {
            black_box(
                glicko2_system
                    .rate(&[glicko2_team1.clone(), glicko2_team2.clone()], &outcome)
                    .unwrap(),
            )
        })
    });
    ...
    group.bench_function("trueskill_simplified", |b| {
        b.iter(|| {
            black_box(
                ts_simplified
                    .rate(&[ts_team1.clone(), ts_team2.clone()], &outcome)
                    .unwrap(),
            )
        })
    });
...
pub fn bench_batch_processing(c: &mut Criterion) {
    ...
    for &game_count in &[100, 1000, 10000] {
        ...
        group.bench_with_input(BenchmarkId::new("elo", game_count), &game_count, |b, _| {
            b.iter(|| {
...
pub fn bench_player_scaling(c: &mut Criterion) {
    ...
    for &player_count in &[2, 4, 8, 16, 32] {
        ...
        if player_count == 2 {
            let ts_system = TrueSkill::new_simplified();
            let ts_ratings =
                data_gen.generate_trueskill_ratings(player_count, SkillDistribution::Normal);
...
pub fn bench_memory_usage(c: &mut Criterion) {
    ...
    group.bench_function("rating_object_creation", |b| {
        b.iter(|| {
            ...
            let _trueskill =
                black_box(TrueSkillRating::new(25.0, (25.0_f64 / 3.0).powi(2)).unwrap());
        })
    });
    ...
    group.bench_function("team_creation", |b| {
        ...
        b.iter(|| black_box(TrueSkillTeam::from_player_ratings(ratings.clone())))
    });

    // Benchmark outcome creation
    group.bench_function("outcome_creation", |b| {
        b.iter(|| black_box(GameOutcome::new(vec![1, 2, 3, 4])))
    });
