        ...
        group.bench_with_input(
            ...
            |b, &player_count| {
                b.iter(|| {
                    ...
                    let mut ratings =
                        data_gen.generate_elo_ratings(player_count, SkillDistribution::Normal);
        ...
        group.bench_with_input(
            ...
            |b, &player_count| {
                b.iter(|| {
                    ...
                    let mut ratings = data_gen
                        .generate_trueskill_ratings(player_count, SkillDistribution::Normal);
                    ...
                    for _round in 0..7 {
                        for i in (0..player_count).step_by(2) {
                            if i + 1 < player_count {
                                let team1 =
                                    TrueSkillTeam::from_player_ratings(vec![ratings[i].clone()]);
                                let team2 = TrueSkillTeam::from_player_ratings(vec![ratings
                                    [i + 1]
                                    .clone()]);
        ...
        group.bench_with_input(
            ...
            |b, &pool_size| {
                b.iter(|| {
                    let elo_system = EloSystem::new();
                    let mut ratings =
                        data_gen.generate_elo_ratings(pool_size, SkillDistribution::Normal);
        ...
        group.bench_with_input(
            ...
            |b, &pool_size| {
                b.iter(|| {
                    let ts_system = TrueSkill::new_simplified();
                    let mut ratings =
                        data_gen.generate_trueskill_ratings(pool_size, SkillDistribution::Normal);

                    // Sort by conservative rating for matchmaking
                    ratings.sort_by(|a, b| {
                        a.conservative_rating()
                            .partial_cmp(&b.conservative_rating())
                            .unwrap()
                    });
                    ...
                    for i in (0..std::cmp::min(100, pool_size)).step_by(2) {
                        if i + 1 < ratings.len() {
                            let team1 =
                                TrueSkillTeam::from_player_ratings(vec![ratings[i].clone()]);
                            let team2 =
                                TrueSkillTeam::from_player_ratings(vec![ratings[i + 1].clone()]);
    ...
    group.bench_function("regular_season", |b| {
        b.iter(|| {
            ...
            let mut teams: Vec<Vec<TrueSkillRating>> = (0..team_count)
                .map(|_| {
                    data_gen.generate_trueskill_ratings(players_per_team, SkillDistribution::Normal)
                })
                .collect();
            ...
            for team_idx in 0..team_count {
                for _match_num in 0..10 {
                    ...
                    if opponent_idx != team_idx {
                        ...
                        let team1_avg: f64 = teams[team_idx].iter().map(|r| r.mean()).sum::<f64>()
                            / players_per_team as f64;
                        let team2_avg: f64 =
                            teams[opponent_idx].iter().map(|r| r.mean()).sum::<f64>()
                                / players_per_team as f64;

                        let outcome = if team1_avg > team2_avg
                            && data_gen.generate_outcomes(1, 2, OutcomePattern::Skill)[0].ranks()[0]
                                == 1
                        {
                            GameOutcome::win(0, 2)
                        } else if team2_avg > team1_avg
                            && data_gen.generate_outcomes(1, 2, OutcomePattern::Skill)[0].ranks()[0]
                                == 2
                        {
                            GameOutcome::win(1, 2)
            ...
            |b, &match_count| {
                b.iter(|| {
                    ...
                    for _ in 0..match_count {
                        ...
                        let outcome =
                            data_gen.generate_outcomes(1, 2, OutcomePattern::Skill)[0].clone();
            ...
            |b, &player_count| {
                b.iter(|| {
                    ...
                    for _ in 0..1000 {
                        // 1000 random matches
                        let idx1 =
                            data_gen.generate_outcomes(1, player_count, OutcomePattern::Balanced)
                                [0]
                            .ranks()[0]
                                % player_count;
