...
impl PlayerManager {
    ...
    pub fn add_match_record(
        ...
        // Validate outcome
        if outcome > 2 {
            return Err(js_error("Invalid outcome: must be 0 (draw), 1 (team1 wins), or 2 (team2 wins)"));
        }
    ...
    pub fn get_player_match_history(
        ...
        let mut matches: Vec<MatchRecord> = player_data.match_ids
            .iter()
    ...
    pub fn export_players(&self, include_inactive: bool) -> Result<String, JsValue> {
        let profiles: Vec<&PlayerProfile> = self.players
            .values()
    ...
    pub fn merge_players(&mut self, from_id: &str, to_id: &str) -> Result<(), JsValue> {
        ...
        let from_match_ids = self.players
            .get(from_id)
    ...
    pub fn add_player_alias(&mut self, player_id: &str, alias: &str) -> Result<(), JsValue> {
        ...
        self.alias_map.insert(alias.to_string(), player_id.to_string());
