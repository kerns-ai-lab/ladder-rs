...
fn bench_wasm_memory_patterns(c: &mut Criterion) {
    ...
    group.bench_function("team_composition_operations", |b| {
        ...
        b.iter(|| {
            let teams: Vec<Vec<String>> =
                player_ids.chunks(5).map(|chunk| chunk.to_vec()).collect();
            black_box(teams)
        })
    });
        ...
        group.bench_with_input(
            ...
            |b, _| {
                let mut ratings: Vec<TrueSkillRating> = (0..20)
                    .map(|i| {
                        TrueSkillRating::new(25.0 + i as f64 * 0.5, (25.0 / 3.0).powi(2)).unwrap()
                    })
                    .collect();
                ...
                b.iter(|| {
                    for &(p1_idx, p2_idx, ref outcome) in &matches {
                        let team1 =
                            TrueSkillTeam::from_player_ratings(vec![ratings[p1_idx].clone()]);
                        let team2 =
                            TrueSkillTeam::from_player_ratings(vec![ratings[p2_idx].clone()]);
