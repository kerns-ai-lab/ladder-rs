...
fn test_rating_calculation_performance() {
    ...
    let systems = vec![
        ...
        (
            "glicko",
            json!({"type": "glicko", "initial_volatility": 0.06}),
        ),
        (
            "trueskill",
            json!({"type": "trueskill", "beta": 4.166666666666667, "tau": 0.08333333333333333}),
        ),
    ];
    ...
    for (system_name, config) in systems {
        ...
        for i in 0..20 {
            let player = system
                .create_player(format!("{}_{}", system_name, i))
        ...
        for i in 0..50 {
            ...
            let updated_teams = system
                .update_ratings(vec![team1, team2])
        ...
        let msg = format!(
            "{} system: 20 players, 50 matches in {:?}",
        ...
        assert!(
            ...
            "{} performance test failed: {:?} > 1000ms",
            system_name,
            duration
        );
    }
...
fn test_stress_conditions() {
    ...
    for i in 0..100 {
        let player = system
            .create_player(format!("stress_player_{}", i))
            .expect("Failed to create player");
        players.push(player);
    }
    ...
    assert!(
        (avg_rating - 1500.0).abs() < 50.0,
        "Average rating drift too large: {}",
        avg_rating
    );

    // Should have some spread in ratings
    let min_rating = players
        .iter()
        .map(|p| p.rating)
    ...
    assert!(
        rating_spread > 50.0,
        "Rating spread too small: {}",
        rating_spread
    );

    let msg = format!(
        "Rating spread: {:.1} (min: {:.1}, max: {:.1}, avg: {:.1})",
...
fn test_memory_usage() {
    ...
    for iteration in 0..10 {
        ...
        for i in 0..20 {
            let player = system
                .create_player(format!("mem_test_{}_{}", iteration, i))
                .expect("Failed to create player");
            players.push(player);
        }
    ...
    web_sys::console::log_1(
        &"Memory usage test completed - created and destroyed 10 systems with 20 players each"
            .into(),
    );
}
