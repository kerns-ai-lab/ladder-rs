...
impl GaussianDistribution {
    ...
    pub fn from_mean_and_variance(mean: f64, variance: f64) -> Self {
        let precision = if variance.is_infinite() {
            0.0
        } else {
            1.0 / variance
        };
    ...
    fn update_message(&mut self, variable_id: usize) -> Result<f64> {
        if variable_id != self.variable_id {
            return Err(Error::InvalidInput(
                "Variable not connected to this factor".to_string(),
            ));
        }
    ...
    fn update_message(&mut self, variable_id: usize) -> Result<f64> {
        if variable_id == self.output_id {
            ...
            let output_precision =
                input_precision * self.coefficient * self.coefficient + 1.0 / self.variance;
            let output_precision_mean = self.message_to_input.precision_mean() * self.coefficient;

            self.message_to_output =
                GaussianDistribution::from_precision_mean(output_precision_mean, output_precision);
            ...
            self.message_to_input =
                GaussianDistribution::from_precision_mean(input_precision_mean, input_precision);
    ...
    fn rate_simplified(
        ...
        for (i, team) in rating_groups.iter().enumerate() {
            ...
            for player in team.player_ratings() {
                ...
                let new_variance =
                    player_variance * (1.0 - update_factor * w * c_squared / c_squared);
    ...
    fn rate_factor_graph(
        ...
        for team_skills in skill_variables.iter() {
            ...
            for &skill_id in team_skills {
                ...
                let linear_factor =
                    Box::new(LinearFactor::new(skill_id, perf_id, 1.0, self.beta_squared));
        ...
        let _convergence_result =
            graph.run_schedule_loop(self.convergence_threshold, self.max_iterations);
...
impl RatingSystem for TrueSkill {
    ...
    fn rate(
        ...
        match self.implementation {
            TrueSkillImplementation::Simplified => self.rate_simplified(rating_groups, outcome),
            TrueSkillImplementation::FactorGraph => self.rate_factor_graph(rating_groups, outcome),
        }
    }
    ...
    fn calculate_match_quality(&self, _rating_groups: &[Self::TeamRating]) -> Result<f64> {
        ...
        Err(Error::Other(
            "TrueSkill match quality calculation not yet implemented".to_string(),
        ))
    }
}
