...
impl Factor for GaussianLikelihoodFactor {
    fn update_message(&mut self, variable_id: usize) -> Result<f64> {
        if variable_id == self.performance_variable_id {
            ...
            let new_message =
                GaussianDistribution::from_precision_mean(0.0, 1.0 / self.beta_squared);
...
impl TrueSkill {
    ...
    fn rate_simplified(
        ...
        let (updated_rating1, updated_rating2) =
            updater.update_ratings(player1_rating, player2_rating, two_player_outcome)?;
    ...
    fn rate_factor_graph(
        ...
        {
            return Err(Error::InvalidInput(
                "Factor graph implementation currently only supports single-player teams"
                    .to_string(),
            ));
        }
        ...
        let _final_delta =
            factor_graph.run_schedule_loop(self.convergence_threshold, self.max_iterations)?;
        ...
        let (final_rating1, final_rating2) =
            updater.update_ratings(player1_rating, player2_rating, two_player_outcome)?;
        ...
        let (v, w) = match outcome {
            ...
            TwoPlayerOutcome::Draw => {
                ...
                if denom.abs() < 1e-10 {
                    ...
                    let w = v * v + ((epsilon - t) * pdf_upper + (epsilon + t) * pdf_lower) / denom;
                    (v, w)
                }
            }
        };
