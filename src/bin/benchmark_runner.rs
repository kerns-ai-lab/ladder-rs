...
impl BaselineEstablisher {
    fn new() -> Self {
        ...
        fs::create_dir_all(&baselines_dir).expect("Failed to create baselines directory");
    ...
    fn collect_criterion_results(
        &self,
    ) -> Result<HashMap<String, HashMap<String, BenchmarkResult>>, Box<dyn std::error::Error>> {
        ...
        println!(
            "📊 Collected results for {} benchmark groups",
            results.len()
        );
        Ok(results)
    }
    ...
    fn parse_benchmark_dir(
        &self,
        bench_dir: &Path,
    ) -> Result<HashMap<String, BenchmarkResult>, Box<dyn std::error::Error>> {
        let mut results = HashMap::new();
            ...
            if entry.file_type()?.is_dir() {
                ...
                if estimates_file.exists() {
                    if let Ok(estimates_content) = fs::read_to_string(&estimates_file) {
                        if let Ok(estimates) =
                            serde_json::from_str::<CriterionEstimates>(&estimates_content)
                        {
    ...
    fn get_performance_targets(&self) -> HashMap<String, PerformanceTarget> {
        ...
        targets.insert(
            "trueskill_simplified".to_string(),
            PerformanceTarget {
        ...
        targets.insert(
            "trueskill_factor_graph".to_string(),
            PerformanceTarget {
    ...
    fn save_baseline(
        ...
        let baseline_file = self
            .baselines_dir
            .join(format!("baseline_{}.json", timestamp_str));
    ...
    fn generate_baseline_report(&self, baseline: &BaselineData) -> String {
        ...
        for (group_name, group_data) in &baseline.measurements {
            ...
            for (bench_name, metrics) in group_data {
                report.push_str(&format!(
                    "- **{}**: {:.2} μs ({:.0} ops/sec)\n",
                    bench_name, metrics.mean_time_us, metrics.throughput_ops_per_sec
                ));
            }
            report.push('\n');
        }
