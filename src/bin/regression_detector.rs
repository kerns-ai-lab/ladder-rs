...
impl RegressionDetector {
    ...
    fn load_baseline(&self, baseline_path: &Path) -> Result<BaselineData, Box<dyn std::error::Error>> {
        let content = fs::read_to_string(baseline_path)?;
        let baseline: BaselineData = serde_json::from_str(&content)?;
        Ok(baseline)
    }

    fn detect_regressions(&self, baseline: &BaselineData, current: &BaselineData) -> Vec<RegressionResult> {
    ...
    fn generate_report(&self, regressions: &[RegressionResult]) -> String {
        ...
        let regressions_found: Vec<_> = regressions.iter()
            .filter(|r| r.is_regression)
            .collect();
        
        let critical_count = regressions_found.iter()
            .filter(|r| r.severity == RegressionSeverity::Critical)
            .count();
        let major_count = regressions_found.iter()
            .filter(|r| r.severity == RegressionSeverity::Major)
            .count();
        let minor_count = regressions_found.iter()
        ...
        if regressions_found.is_empty() {
            ...
            report.push_str(&format!("⚠️ **{} performance regressions detected out of {} benchmarks**\n\n", 
                regressions_found.len(), total_benchmarks));
        ...
        report.push_str(&format!("- Major threshold: {:.1}%\n", self.major_threshold * 100.0));
        report.push_str(&format!("- Critical threshold: {:.1}%\n", self.critical_threshold * 100.0));
        report.push_str("\n");
        ...
        if !regressions_found.is_empty() {
            ...
            for regression in &regressions_found {
                ...
                report.push_str(&format!("- Baseline: {:.0} ops/sec\n", regression.baseline_throughput));
                report.push_str(&format!("- Current: {:.0} ops/sec\n", regression.current_throughput));
        ...
        for result in regressions {
            grouped_results.entry(result.group_name.clone())
        ...
        for (group_name, group_results) in grouped_results {
            ...
            for result in group_results {
                ...
                report.push_str(&format!("{} **{}**: {:.0} ops/sec ({:+.1}%)\n",
                    status,
    ...
    fn should_fail_ci(&self, regressions: &[RegressionResult]) -> bool {
        ...
        let critical_count = regressions.iter()
            .filter(|r| r.severity == RegressionSeverity::Critical)
            .count();
        let major_count = regressions.iter()
            .filter(|r| r.severity == RegressionSeverity::Major)
            .count();
    ...
    fn run_analysis(&self, baseline_path: &Path, current_path: &Path) -> Result<bool, Box<dyn std::error::Error>> {
        ...
        println!("📈 Analyzing {} benchmark groups...", current.measurements.len());
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !baseline_path.exists() {
        eprintln!("Error: Baseline file does not exist: {}", baseline_path.display());
        std::process::exit(1);
    }

    if !current_path.exists() {
        eprintln!("Error: Current file does not exist: {}", current_path.display());
        std::process::exit(1);
    }
