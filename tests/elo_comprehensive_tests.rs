...
fn test_elo_error_conditions() {
    ...
    let multi_player_team =
        EloTeamRating::from_player_ratings(vec![EloRating::new(1500.0), EloRating::new(1600.0)]);
...
fn test_elo_system_consistency() {
    ...
    assert_eq!(
        result1[0].player_ratings()[0].rating(),
        result2[0].player_ratings()[0].rating()
    );
    assert_eq!(
        result1[1].player_ratings()[0].rating(),
        result2[1].player_ratings()[0].rating()
    );
}
