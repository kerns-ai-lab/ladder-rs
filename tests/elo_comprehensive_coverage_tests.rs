...
fn test_elo_extreme_rating_differences() {
    ...
    assert!(
        rating_change.abs() < 2.0,
        "Expected result should cause minimal rating change"
    );
    ...
    assert!(
        low_gain.abs() > 0.5,
        "Upset should cause measurable rating change"
    );
    assert!(
        high_loss.abs() > 0.5,
        "Upset should cause measurable rating change"
...
fn test_elo_custom_parameters() {
    ...
    let stable_result = low_k_system
        .rate(&[stable_team1, stable_team2], &outcome)
        .unwrap();

    let stable_change = stable_result[0].player_ratings()[0].rating() - 1500.0;
    assert!(
        stable_change < rating_change,
...
fn test_elo_rating_sequence_convergence() {
    ...
    for _ in 0..100 {
        // More iterations for meaningful convergence
    ...
    assert!(
        player1_rating.rating() > 1520.0,
        "Consistent winner should have higher rating"
    );
    assert!(
        player2_rating.rating() < 1480.0,
        "Consistent loser should have lower rating"
    );
    assert!(
        player1_rating.rating() - player2_rating.rating() > 30.0,
        "Rating difference should be significant"
    );
}
...
fn test_elo_win_probability_edge_cases() {
    ...
    assert!(
        (quality_equal - 1.0).abs() < 0.01,
        "Equal players should have maximum quality"
    );

    // Test with very different ratings (should be low quality)
    let quality_different = system
        .calculate_match_quality(&[
    ...
    assert!(
        quality_different < 0.3,
        "Very different players should have low quality"
    );
}
...
fn test_elo_draw_scenarios() {
    ...
    if rating1_change.abs() > 0.1 && rating2_change.abs() > 0.1 {
        assert!(
            rating1_change <= 0.0,
            "Higher rated player shouldn't gain much in draw"
        );
        assert!(
            rating2_change >= 0.0,
            "Lower rated player shouldn't lose much in draw"
        );
    }

    // Test that draws don't cause huge rating swings
    assert!(
        rating1_change.abs() < 50.0,
        "Draw shouldn't cause huge rating changes"
    );
    assert!(
        rating2_change.abs() < 50.0,
        "Draw shouldn't cause huge rating changes"
    );
}
