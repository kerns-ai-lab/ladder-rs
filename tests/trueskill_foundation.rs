use ladder_rs::{
    ...
    trueskill::{TrueSkill, TrueSkillImplementation, TrueSkillRating},
};
...
fn test_rating_system_implementation() {
    ...
    assert!(
        rate_result.is_ok(),
        "Rate method should succeed for valid two-player games"
    );
    ...
    assert!(
        quality_result.is_err(),
        "Match quality calculation should still return error"
    );
}
