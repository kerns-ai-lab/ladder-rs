...
fn test_very_certain_players() {
    ...
    assert!(
        (updated_strong.mean() - 30.0).abs() < 1.0,
        "Certain strong player should change little"
    );
    assert!(
        (updated_weak.mean() - 20.0).abs() < 1.0,
        "Certain weak player should change little"
    );
}
...
fn test_very_uncertain_players() {
    ...
    assert!(
        updated_winner.variance() < 1000.0,
        "Winner variance should decrease"
    );
    assert!(
        updated_loser.variance() < 1000.0,
        "Loser variance should decrease"
    );
}
...
fn test_multiple_games_sequence() {
    ...
    for i in 0..10 {
        ...
        assert!(
            team1.player_ratings()[0].variance() > 0.0,
            "Team 1 variance should remain positive"
        );
        assert!(
            team2.player_ratings()[0].variance() > 0.0,
            "Team 2 variance should remain positive"
        );
    }
...
fn test_alternating_wins() {
    ...
    println!(
        "Final ratings: Player 1: {:.2}, Player 2: {:.2}, Diff: {:.2}",
        final_rating1.mean(),
        final_rating2.mean(),
        rating_diff
    );
    ...
    assert!(
        rating_diff < 4.0,
        "After alternating wins, players should have reasonably similar ratings (diff: {:.2})",
        rating_diff
    );
    ...
    assert!(
        final_rating1.variance() < initial_variance,
        "Player 1 variance should decrease"
    );
    assert!(
        final_rating2.variance() < initial_variance,
...
fn test_draw_sequence() {
    ...
    assert!(
        (final_rating1.mean() - 25.0).abs() < 1.0,
        "Player 1 should stay near initial rating"
    );
    assert!(
        (final_rating2.mean() - 25.0).abs() < 1.0,
        "Player 2 should stay near initial rating"
    );
