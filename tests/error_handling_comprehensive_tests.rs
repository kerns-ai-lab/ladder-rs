...
mod tests {
    ...
    fn test_numerical_error() {
        ...
        assert_eq!(error.to_string(), "Numerical precision error: precision loss detected");
    ...
    fn test_convergence_failure_error() {
        let error = Error::ConvergenceFailure("algorithm failed to converge after 1000 iterations".to_string());
        
        assert_eq!(error.to_string(), "Failed to converge: algorithm failed to converge after 1000 iterations");
    ...
    fn test_invalid_configuration_error() {
        ...
        assert_eq!(error.to_string(), "Invalid configuration: beta parameter must be positive");
    ...
    fn test_invalid_outcome_error() {
        let error = Error::InvalidOutcome("ranks vector length does not match team count".to_string());
        
        assert_eq!(error.to_string(), "Invalid outcome: ranks vector length does not match team count");
    ...
    fn test_error_messages_formatting() {
        ...
        for error in errors {
            ...
            match error {
                ...
                Error::InvalidConfiguration(_) => assert!(msg.starts_with("Invalid configuration:")),
    ...
    fn test_functional_error_handling_patterns() {
        ...
        fn square_root(x: f64) -> Result<f64> {
            if x < 0.0 {
                Err(Error::CalculationError("square root of negative number".to_string()))
            } else {
                Ok(x.sqrt())
            }
        }
