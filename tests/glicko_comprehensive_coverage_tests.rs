...
fn test_glicko_rating_period_updates() {
    ...
    assert!(updated_rating.rd < inactive_player.rd, "RD should decrease after games");
    assert!(updated_rating.mu > inactive_player.mu, "Rating should increase after win");
}
...
fn test_glicko_custom_configuration() {
    ...
    let custom_result = custom_glicko.rate(&[team1.clone(), team2.clone()], &outcome).unwrap();
...
fn test_glicko2_volatility_updates() {
    ...
    assert!(current_rating.volatility < stable_player.volatility, 
            "Consistent performance should reduce volatility");
    assert!(current_rating.rd < stable_player.rd, 
...
fn test_glicko2_custom_configuration() {
    ...
    let custom_result = custom_glicko2.rate(&[team1.clone(), team2.clone()], &outcome).unwrap();
    ...
    assert_ne!(custom_volatility, default_volatility, 
...
fn test_glicko_extreme_rd_values() {
    ...
    assert!(new_change > exp_change, 
            "Player with higher RD should have larger rating change");
    
    // RD should decrease for new player
    assert!(updated_new.rd < new_player.rd, "RD should decrease after game");
}
...
fn test_glicko_error_conditions() {
    ...
    let team = GlickoTeamRating::from_player_ratings(vec![
        GlickoRating::new(1500.0, 100.0)
    ]);
...
fn test_glicko2_error_conditions() {
    ...
    // Test invalid team counts
    let team = Glicko2TeamRating::from_player_ratings(vec![
        Glicko2Rating::new(1500.0, 100.0, 0.06)
    ]);
    ...
    assert!(result.is_err(), "Multiple teams should cause error for Glicko2");
}
...
fn test_glicko_match_quality_edge_cases() {
    ...
    assert!(quality > 0.9, "Identical players should have high match quality");
    ...
    let quality_diff = glicko.calculate_match_quality(&[team_high, team_low]).unwrap();
    assert!(quality_diff < 0.3, "Very different players should have low match quality");
}
...
fn test_glicko_draw_handling() {
    ...
    assert!(updated1.mu < player1.mu, "Higher rated player should lose rating in draw");
    assert!(updated2.mu > player2.mu, "Lower rated player should gain rating in draw");
}
