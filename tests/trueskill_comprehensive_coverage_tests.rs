use ladder_rs::{
    ...
    trueskill::{
        FactorGraph, GaussianDistribution, GaussianPriorFactor, TrueSkill, TrueSkillImplementation,
...
fn test_trueskill_custom_parameters() {
    ...
    assert_eq!(
        player1.variance(),
        100.0,
        "Custom sigma_0_squared should be used"
    );
    ...
    let default_result = default_ts
        .rate(&[default_team1, default_team2], &outcome)
        .unwrap();
...
fn test_trueskill_parameter_validation() {
    ...
    assert!(
        result.is_err(),
        "Negative sigma_0_squared should be rejected"
    );
    ...
    assert!(
        result.is_err(),
        "Draw_probability of 1.0 should be rejected"
    );
}
...
fn test_trueskill_draw_probability_effects() {
    ...
    let low_draw_result = low_draw_ts
        .rate(&[team1.clone(), team2.clone()], &draw_outcome)
        .unwrap();
    ...
    assert!(
        low_draw_rating.variance() != high_draw_rating.variance(),
...
fn test_trueskill_implementation_differences() {
    ...
    assert!(
        simplified_result.is_ok(),
        "Simplified implementation should work"
    );

    let factor_graph_result = factor_graph_ts.rate(&[team1, team2], &outcome);
    assert!(
        factor_graph_result.is_ok(),
        "Factor graph implementation should work"
    );
}
...
fn test_trueskill_rating_convergence() {
    ...
    assert!(
        player1.variance() < (25.0_f64 / 3.0).powi(2),
        "Variance should decrease with more games"
    );
    assert!(
        player2.variance() < (25.0_f64 / 3.0).powi(2),
        "Variance should decrease with more games"
    );
}
...
fn test_trueskill_gaussian_distribution() {
    ...
    assert!(
        product.mean() != gaussian1.mean(),
        "Multiplication should change distribution"
    );

    // Test division
    let quotient = gaussian1.divide(&gaussian2);
    assert!(
        quotient.mean() != gaussian1.mean(),
        "Division should change distribution"
    );
    ...
    assert!(
        diff > 0.0,
        "Different distributions should have positive difference"
    );
...
fn test_trueskill_factor_graph_components() {
    ...
    fg.add_factor(Box::new(
        GaussianPriorFactor::new(var1_id, 5.0, 1.0).unwrap(),
    ));
    fg.add_factor(Box::new(
        GaussianPriorFactor::new(var2_id, 3.0, 2.0).unwrap(),
    ));
...
fn test_trueskill_match_quality_not_implemented() {
    ...
    assert!(
        result.is_err(),
        "Match quality calculation should not be implemented yet"
    );
}
