...
fn test_wasm_cargo_toml_configuration() {
    ...
    let content = fs::read_to_string(cargo_toml_path)
        .expect("Should be able to read wasm/Cargo.toml");
    
    // Parse as TOML to validate syntax
    let toml_value: toml::Value = toml::from_str(&content)
        .expect("wasm/Cargo.toml should be valid TOML");
    ...
    let lib = toml_value.get("lib")
        .expect("wasm/Cargo.toml should have [lib] section for WASM");
    
    let crate_type = lib.get("crate-type")
...
fn test_wasm_dependencies() {
    ...
    let dependencies = toml_value.get("dependencies")
    ...
    for dep in required_deps {
        assert!(
            ...
            "Should include {} dependency", dep
        );
    }
    
    // Check for ladder-rs dependency with proper path
    let ladder_rs_dep = dependencies.get("ladder-rs")
...
fn test_feature_flags() {
    ...
    if let Some(features) = toml_value.get("features") {
        ...
        if features.get("wee_alloc").is_some() {
            let wee_alloc_features = features.get("wee_alloc")
                .and_then(|f| f.as_array())
                .expect("wee_alloc feature should be an array");
            
            assert!(
                wee_alloc_features.iter().any(|f| f.as_str() == Some("dep:wee_alloc")),
...
fn test_wasm_lib_rs_basic_structure() {
    ...
    let content = fs::read_to_string(lib_rs_path)
        .expect("Should be able to read wasm/src/lib.rs");
...
fn test_cargo_workspace_integration() {
    ...
    let content = fs::read_to_string(main_cargo_toml)
    ...
    let _toml_value: toml::Value = toml::from_str(&content)
        .expect("Root Cargo.toml should remain valid TOML");
