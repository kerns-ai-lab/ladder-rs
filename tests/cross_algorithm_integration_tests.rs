use ladder_rs::{
    ...
    glicko::{GlickoRating, Glicko2Rating, Glicko, Glicko2, GlickoTeamRating, Glicko2TeamRating},
...
fn test_simple_match_across_systems() {
    ...
    let glicko_result = glicko.rate(&[glicko_team1, glicko_team2], &outcome).unwrap();
    let glicko2_result = glicko2.rate(&[glicko2_team1, glicko2_team2], &outcome).unwrap();
...
fn test_match_quality_across_systems() {
    ...
    let elo_quality = elo.calculate_match_quality(&[elo_team1, elo_team2]).unwrap();
    let glicko_quality = glicko.calculate_match_quality(&[glicko_team1, glicko_team2]).unwrap();
    let glicko2_quality = glicko2.calculate_match_quality(&[glicko2_team1, glicko2_team2]).unwrap();
...
fn test_convergence_across_systems() {
    ...
    println!("Glicko: P1={:.3}, P2={:.3}", glicko_p1.mean(), glicko_p2.mean());
    println!("Glicko2: P1={:.3}, P2={:.3}", glicko2_p1.mean(), glicko2_p2.mean());
...
fn test_draw_handling() {
    ...
    let ts_team1 = TrueSkillTeam::from_player_ratings(vec![trueskill.create_rating_with_values(1500.0, 200.0 * 200.0)]);
    let ts_team2 = TrueSkillTeam::from_player_ratings(vec![trueskill.create_rating_with_values(1500.0, 200.0 * 200.0)]);
    ...
    let glicko_result = glicko.rate(&[glicko_team1, glicko_team2], &draw_outcome).unwrap();
    let glicko2_result = glicko2.rate(&[glicko2_team1, glicko2_team2], &draw_outcome).unwrap();
    let ts_result = trueskill.rate(&[ts_team1, ts_team2], &draw_outcome).unwrap();
